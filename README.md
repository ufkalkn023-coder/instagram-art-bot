...
| `INSTAGRAM_ACCESS_TOKEN` | Production publish ve Insights için gerekli | Meta Graph API erişim token’ı; v2 feed/Reel workflow'ları bunu `INSTAGRAM_ACCESS_TOKEN_V2`, Insights workflow'u ise `INSTAGRAM_INSIGHTS_ACCESS_TOKEN` repository secret'ından alır |
...
Varsayılan mod R2'ye yazmaz. `--output-dir` yalnız yerel dosya üretir. Üretim yüklemesi ayrı yetki, `--write --target-bucket <durable-state-bucket> --confirm-production-write I_AUTHORIZE_PRODUCTION_RECOVERY_BOOTSTRAP` ve ayrıca `--lifecycle-audit-source` gerektirir; bu komut normal geliştirme/CI doğrulamasında çalıştırılmaz. `dashboard` seçeneğinde operatör hedef bucket'ın lifecycle kurallarını Cloudflare Dashboard'da denetleyip nesne silme/expiration kuralı bulunmadığını doğruladıktan sonra `--confirm-dashboard-lifecycle I_VERIFIED_NO_OBJECT_EXPIRATION_FOR_<durable-state-bucket>` verir. `control-plane-s3` seçeneğinde yalnız ayrı `CLOUDFLARE_R2_CONTROL_PLANE_*` credential'ları kullanılarak lifecycle API'si okunur; scoped state key'i bu audit için kullanılmaz. Default 7 günlük incomplete-multipart-abort kuralı kabul edilir; nesne expiration kuralı bootstrap'ı engeller. Audit ayrıca sonraki operasyonlarda tekrarlanmalıdır: nesnelerde `x-amz-expiration` görülmemesi, bucket kuralının güvenli olduğunu tek başına kanıtlamaz. Bootstrap iki nesnenin de yokluğunu doğrular, receipt nesnesini önce, safety nesnesini en son create-only koşuluyla oluşturur. Herhangi bir belirsiz veya kısmi sonuçta otomatik tekrar denenmez; operatör iki nesneyi okuyup durumu incelemelidir. Kaynak forensic JSON, eski `data/posted_history.json` veya örnek şemalar doğrudan production'a yüklenmez.

Recovery rollout sırası: feed ve Reel schedule değişkenlerini kapalı tut; feed ve Reel GitHub workflow'larını devre dışı bırakıp queued/in-progress job kalmadığını doğrula. Schedule değişkeni manuel dispatch'i engellemez. GitHub rerun'ları ilk koşunun commit SHA'sını kullandığı için eski workflow'lara verilen `INSTAGRAM_ACCESS_TOKEN` repository secret'ını bootstrap öncesinde kaldır; önce v2 feed/Reel için `INSTAGRAM_ACCESS_TOKEN_V2` ve Insights için `INSTAGRAM_INSIGHTS_ACCESS_TOKEN` repository secret'larını güvenli şekilde hazırla. Eski token'ı almış job kalmadığını yeniden doğrula. Ayrı private durable-state bucket'ı silme lifecycle kuralı olmadan ve ayrı scoped credential'larla hazırla, v2 kodu `main` üzerine ayrı yetkiyle merge et, forensic bootstrap için ayrı yetki al, iki nesneyi doğrula, read-only preflight ve izole R2 testlerini tamamla. Yalnız bundan sonra workflow'ları yeniden açmak ve schedule/canary için ayrı operatör kararı verilir. Eski SHA rerun'ları eski secret yokken Instagram'a yazamaz; kısmi dağıtımda yeni kod eksik v2 state ile durur. Stale v2 PENDING lock yalnız açık reconciliation/expiry CAS ile kaldırılır ve v2 feed worker'ı reservation ID ile fence edilir.
